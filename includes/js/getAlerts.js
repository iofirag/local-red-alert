	...
	*************************************************************************************************************************************************************************************************************************/
	...
	//****************************************
					...
					if ((areas.length > 0) && ((alertItem == null) || ((currAlertId != alertItem.alertId) && (areas.length > 0)))) {
						...
						for (var i = 0; i < alertItem.areaList.length; i++) {
							for (var j = 0; j < alertItem.areaList[i].placeList.length; j++) {
								...
								if (KmDistanceBetweenTwoLatLon <= 4) {
									...
									var popupRed = window.open('red.html', 'Red-Alert in your location',            'width='+popupRedW+', height='+popupRedH+', top='+top+', left='+left+ ',toolbar=0,menubar=0,location=0,status=1,scrollbars=1,resizable=1');
									setTimeout(function() {
										popupRed.close();
						...
						if(nearUser ==2){
							/* Play alarm sound */
							alarm = new Audio("includes/noise/piano.mp3");
							if (alarm) alarm.play();
							...
							/* Play alarm sound */
							alarm = new Audio("includes/noise/ding.mp3");
							if (alarm) alarm.play();
